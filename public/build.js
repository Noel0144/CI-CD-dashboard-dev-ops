(function () {
  ...
  async function fetchBuilds() {
    try {
      const res = await fetch(`/api/github/repos/${getRepo()}/actions/runs?per_page=15`, { headers: HEADERS });
      if (!res.ok) return [];
      ...
      return data.workflow_runs.map((run) => {
        ...
        return { id: run.run_number, branch: run.head_branch, status, duration: durationSecs > 0 ? durationSecs : 0 };
      });
    } catch (e) {
  ...
  function initCharts(builds) {
    ...
    donutChart = new Chart(document.getElementById("buildDonutChart"), {
      type: "doughnut",
      data: {
    ...
    barChart = new Chart(document.getElementById("buildBarChart"), {
      type: "bar",
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: builds.map((b) => b.status === "Failed" ? "#ff4d4d" : b.status === "Running" ? "#f5c518" : "#32e6e2"),
          borderRadius: 5,
        }],
      },
  ...
  function updateKPIs(builds) {
    ...
    const running = builds.filter((b) => b.status === "Running").length;
    let avgDur = 0;
    if (builds.length > 0) avgDur = Math.floor(builds.reduce((sum, b) => sum + b.duration, 0) / builds.length);
    ...
    document.getElementById("kpi-avg-dur").textContent = mins > 0 ? `${mins}m ${secs}s` : `${secs}s`;
  }
  ...
  function buildTable(builds) {
    ...
    tbody.innerHTML = "";
    builds.forEach((b) => {
      const pill = b.status === "Passed" ? "status-success" : b.status === "Failed" ? "status-error" : "status-warning";
  ...
  function toggleDetail(btn) {
    const panel = document.getElementById("buildDetailPanel");
  ...
  async function init() {
    ...
    if (builds.length > 0) {
      ...
      ["kpi-total","kpi-pass","kpi-fail","kpi-run","kpi-avg-dur"].forEach(id => {
        document.getElementById(id).textContent = "0";
      });
    }
  }

  window.initBuild       = init;
  window.toggleBuildDetail = toggleDetail;
})();