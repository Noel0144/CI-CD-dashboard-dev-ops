(function () {
  ...
  async function fetchHealthData() {
    try {
      const issueRes = await fetch(`/api/github/repos/${getRepo()}/issues?state=open&per_page=5`, { headers: HEADERS });
      let issues = [];
      ...
      if (rlRes.ok) rateLimit = await rlRes.json();
      ...
      if (alerts.length === 0) {
        alerts.push({ title: "System Stable", severity: "success", message: "No open issues detected in the repository.", timestamp: "Last checked just now" });
      }
      return { services: metrics, alerts };
    } catch (e) {
  ...
  function initCharts(data) {
    ...
    document.getElementById("health-kpi-0").textContent = services.length;
    document.getElementById("health-kpi-1").textContent = healthy;
    document.getElementById("health-kpi-2").textContent = warning;
    document.getElementById("health-kpi-3").textContent = critical;
    document.getElementById("health-kpi-4").textContent = "99.98%";

    if (donutChart) donutChart.destroy();
    donutChart = new Chart(document.getElementById("healthDonutChart"), {
      type: "doughnut",
      data: {
        labels: ["Healthy", "Warning", "Critical"],
        datasets: [{ data: [healthy, warning, critical], backgroundColor: ["#0ceb0c", "#f5c518", "#ff4d4d"], borderWidth: 2, borderColor: "#1a2535" }],
      },
    ...
    barChart = new Chart(document.getElementById("healthBarChart"), {
      type: "bar",
      data: {
        labels: services.map((s) => s.name.substring(0, 10)),
        datasets: [{ label: "Load / Utilization", data: [45, 12, 8, 30], backgroundColor: ["#32e6e2", "#32e6e2", "#f5c518", "#32e6e2"], borderRadius: 5 }],
      },
      options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { ticks: { color: "#aaa", font: { size: 12 } }, grid: { color: "#1e2a38" } }, y: { ticks: { color: "#aaa", font: { size: 12 } }, grid: { color: "#1e2a38" } }, max: 100 } },
    });
  }
  ...
  function buildAlerts(alerts) {
    ...
    container.innerHTML = "";
    const badge = document.querySelector(".toggle-btn-alert .alert-badge");
    if (badge) badge.textContent = alerts.filter((a) => a.severity !== "success").length;
    alerts.forEach((a) => {
      ...
      container.innerHTML += `<div class="alert-item" style="border-left:4px solid ${borderColor};">
        <div class="alert-header"><strong>${a.title}</strong><span class="status-pill ${pillClass}">${a.severity.toUpperCase()}</span></div>
  ...
  function togglePanel(panelId, btn) {
    ...
    if (panelId === "servicePanel") {
      btn.innerHTML = (open ? "+" : "&#11015;") + " Service Details";
    } else {
      const cnt = document.querySelector(".alert-badge") ? document.querySelector(".alert-badge").textContent : "0";
      btn.innerHTML = `${open ? "+" : "&#9888;"} Active Alerts <span class="alert-badge">${cnt}</span>`;
    }
  }
  ...
  window.toggleHealthPanel = togglePanel;
})();