(function () {
  ...
  async function fetchPipelineData() {
    try {
      const res = await fetch(`/api/github/repos/${getRepo()}/actions/runs?per_page=10`, { headers: HEADERS });
      if (!res.ok) return { runs: [], stages: [], history: [], rawRuns: [] };
      const data = await res.json();
      const workflowRuns = data.workflow_runs;
      let runsArray = [], historyArray = [], allJobs = [];
      ...
      const runPromises = workflowRuns.slice(0, 5).map(async (run) => {
        ...
        if (jobsRes.ok) {
          ...
          runsArray.push({ id: "#" + run.run_number, branch: run.head_branch, by: run.actor.login, ago: agoStr, commit: run.head_sha.substring(0, 7), status: runStatus, stages: stagesList, chips: [`Duration: ${mins}m ${secs}s`, `Workflow: ${run.name}`] });
          let failedJob = "—";
          const failedJobs = jobsData.jobs.filter((j) => j.conclusion === "failure");
          if (failedJobs.length > 0) failedJob = failedJobs[0].name;
          historyArray.push({ run: "#" + run.run_number, branch: run.head_branch, status: runStatus, duration: `${mins}m ${secs}s`, failed: failedJob });
        }
      });
      ...
      let stageStats = {};
      allJobs.forEach((job) => {
        if (!stageStats[job.name]) stageStats[job.name] = { total: 0, failed: 0 };
      ...
      const stagesArray = Object.keys(stageStats).map((name) => {
        ...
        return { name, rate: parseFloat(failureRate), color: parseFloat(failureRate) > 0 ? "#ff4d4d" : "#32e6e2" };
      });
  ...
  function initCharts(data) {
    ...
    document.getElementById("pipeline-kpi-0").textContent = rawRuns.length;
    const successRate = rawRuns.length > 0 ? ((passed / rawRuns.length) * 100).toFixed(1) : 0;
    document.getElementById("pipeline-kpi-1").textContent = `${successRate}%`;
    document.getElementById("pipeline-kpi-2").textContent = failed;
    document.getElementById("pipeline-kpi-3").textContent = running;

    if (donutChart) donutChart.destroy();
    donutChart = new Chart(document.getElementById("pipelineDonutChart"), {
      type: "doughnut",
      data: {
        labels: ["Passed", "Failed", "Running"],
        datasets: [{ data: [passed, failed, running], backgroundColor: ["#0ceb0c", "#ff4d4d", "#f5c518"], borderWidth: 2, borderColor: "#1a2535" }],
      },
    ...
    barChart = new Chart(document.getElementById("pipelineBarChart"), {
      type: "bar",
      data: {
        labels: data.stages.map((s) => s.name),
        datasets: [{ label: "Failure Rate %", data: data.stages.map((s) => s.rate), backgroundColor: data.stages.map((s) => s.color), borderRadius: 5 }],
      },
      options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { ticks: { color: "#aaa", font: { size: 11 } }, grid: { color: "#1e2a38" } }, y: { ticks: { color: "#aaa", font: { size: 12 }, callback: (v) => v + "%" }, grid: { color: "#1e2a38" } } } },
    });
  }
  ...
  function buildStageBreakdown(stages) {
    ...
    stages.forEach((s) => {
      ...
      container.innerHTML += `<div class="stage-stat-card ${isHighest ? "stage-stat-card--warn" : ""}">
  ...
  function buildRuns(runs) {
    ...
    runs.forEach((r) => {
      ...
      container.innerHTML += `<div class="pl-run-card" style="border-left-color:${borderColor};">
  ...
  window.initPipeline       = init;
  window.togglePipelinePanel = togglePanel;
})();